...
ws2812_pixel_struct_t pixel_arr[8] = {
...
void ws2812_data_TransTo_frame(ws2812_pixel_struct_t const *pixel_arr, ws2812_frame_struct_t* frame, uint32_t pixel_num)
{
	...
	if(pixel_num <= 8 && pixel_arr != 0 && frame != 0)	
		{ }
	else
		{return;}
	...
	for(i = 0; i < pixel_num; i++)
	{
		...
		for(j = 0; j < 8; j++)
		{
			frame->data[k+j] = 			((pixel_arr[i].B & (0x80U>>j))?65U:35U);

			
			frame->data[k+j+8] = 			((pixel_arr[i].R & (0x80U>>j))?65U:35U);

			
			frame->data[k+j+16] = 			((pixel_arr[i].G & (0x80U>>j))?65U:35U);
...
void ws2812_update(void)
{
	ws2812_data_TransTo_frame(pixel_arr, &ws2812_frame_buffer, 8);
	if(HAL_TIM_PWM_Start_DMA(&htim3,0x0000000CU,(uint32_t *)&ws2812_frame_buffer,41+24*8) != HAL_OK)
	{
		Error_Handler();
	}
}
