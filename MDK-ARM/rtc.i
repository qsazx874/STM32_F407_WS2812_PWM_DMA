...
void MX_RTC_Init(void)
{
  ...
  sTime.Minutes = 0x0;
  sTime.Seconds = 0x0;
  sTime.DayLightSaving = 0x00000000U;
  sTime.StoreOperation = 0x00000000U;
  if (HAL_OK != HAL_OK)
  {
    Error_Handler();
  }
  sDate.WeekDay = ((uint8_t)0x01);
  sDate.Month = ((uint8_t)0x01);
  sDate.Date = 0x1;
  sDate.Year = 0x0;
  ...
  if (HAL_RTCEx_SetWakeUpTimer_IT(&hrtc, 0, 0x00000004U) != HAL_OK)
  {
    Error_Handler();
  }
...
void HAL_RTCEx_WakeUpTimerEventCallback(RTC_HandleTypeDef *hrtc)
{
	...
	HAL_RTC_GetTime(hrtc,&mytime,0x00000000U);
	HAL_RTC_GetDate(hrtc,&mydate,0x00000000U);
	
	printf("%04d-%02d-%02d\n",mydate.Year+2000,mydate.Month,mydate.Date);
	printf("%02d:%02d:%02d\n",mytime.Hours,mytime.Minutes,mytime.Seconds);
	printf("------------------\n\n");
	HAL_GPIO_TogglePin(((GPIO_TypeDef *) ((0x40000000UL + 0x00020000UL) + 0x0000UL)),((uint16_t)0x0080));
}
